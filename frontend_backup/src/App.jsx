...
const CONNECTION_PLANS = [
  ...
  { id: 'premium', name: 'Premium Connect', price: 999, currency: 'INR',
  ...
  { id: 'enterprise', name: 'Enterprise Connect', price: 2499, currency: 'INR',
...
const Hero = ({ onScrollToPlans }) => (
  <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28 text-center relative">
      ...
      <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-slate-800 leading-[1.1] tracking-tight animate-fade-up delay-100">
...
const PaymentModal = ({ plan, isOpen, onClose, onPaymentSuccess, addNotification }) => {
  ...
  const handleCardNumberChange = (e) => {
    ...
    if (raw.length <= 16) setCardNumber(formatCardNumber(raw));
  };
  ...
  const validateForm = () => {
    ...
    if (expiry.replace('/', '').length < 4) newErrors.expiry = 'Invalid expiry date';
  ...
  if (!response.ok) {
    addNotification('error', 'Error', 'Could not start payment. Please try again.');
    return;
  }
        ...
        <div className="p-6">
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" placeholder="1234 5678 9012 3456" value={cardNumber} onChange={handleCardNumberChange} className={`w-full px-4 py-3 rounded-xl stripe-input bg-slate-50 text-slate-800 placeholder:text-slate-400 ${errors.cardNumber ? 'error' : ''}`} />
                  {errors.cardNumber && <p className="mt-1 text-xs text-red-500">{errors.cardNumber}</p>}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Expiry</label>
                    <input type="text" placeholder="MM/YY" value={expiry} onChange={handleExpiryChange} className={`w-full px-4 py-3 rounded-xl stripe-input bg-slate-50 text-slate-800 placeholder:text-slate-400 ${errors.expiry ? 'error' : ''}`} />
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="123" value={cvc} onChange={handleCvcChange} className={`w-full px-4 py-3 rounded-xl stripe-input bg-slate-50 text-slate-800 placeholder:text-slate-400 ${errors.cvc ? 'error' : ''}`} />
                    {errors.cvc && <p className="mt-1 text-xs text-red-500">{errors.cvc}</p>}
                  </div>
                </div>
                <button type="submit" className="w-full mt-2 py-4 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all hover:scale-105 flex items-center justify-center gap-2">
                  <i className="fa-solid fa-lock text-sm"></i> Pay {formatCurrency(plan.price)}
                </button>
          ...
          {step === 'processing' && (
            <div className="py-8 text-center">
              ...
              <h4 className="text-lg font-semibold text-slate-800">Processing Payment...</h4>
              ...
              <div className="mt-6 w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
                <div className="h-full bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full transition-all duration-200" style={{ width: `${progress}%` }}></div>
              </div>
